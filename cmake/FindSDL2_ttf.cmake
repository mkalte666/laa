...
#
# SDL2::SDL2_ttf - Imported target to use for building a library SDL2_TTF_FOUND
# - True if SDL2 was found. SDL2_TTF_DYNAMIC - If we found a DLL version of SDL
# (meaning you might want to copy a DLL from SDL2::SDL2_ttf)
#
# 2019 Malte Kießling <mkalte@mkalte.me>: Added support for SDL2_TTF
#
# Original Author: 2015 Ryan Pavlik <ryan.pavlik@gmail.com> <abiryan@ryand.ttf>
#
...
if(PKG_CONFIG_FOUND)
    pkg_search_module(SDL2PC QUIET sdl2_TTF)
    if(SDL2PC_INCLUDE_DIRS)
        set(SDL2_TTF_INCLUDE_HINTS ${SDL2PC_INCLUDE_DIRS})
    endif()
    if(SDL2PC_LIBRARY_DIRS)
        set(SDL2_TTF_LIB_HINTS ${SDL2PC_LIBRARY_DIRS})
    endif()
endif()
...
if(APPLE AND "${SDL2_TTF_LIBRARY}" MATCHES "(/[^/]+)*.framework(/.*)?$")
    set(_SDL2_TTF_framework TRUE)
    set(SDL2_TTF_FRAMEWORK "${SDL2_TTF_LIBRARY}")
    # Move up in the directory tree as required to get the framework directory.
    while("${SDL2_TTF_FRAMEWORK}" MATCHES "(/[^/]+)*.framework(/.*)$"
          AND NOT "${SDL2_TTF_FRAMEWORK}" MATCHES "(/[^/]+)*.framework$")
        get_filename_component(SDL2_TTF_FRAMEWORK "${SDL2_TTF_FRAMEWORK}"
                               DIRECTORY)
    endwhile()
...
if(WIN32 AND SDL2_TTF_LIBRARY)
    ...
    file(GLOB SDL2_TTF_RUNTIME_LIBRARY_ADDITIONALS
         ${SDL2_TTF_RUNTIME_LIBRARY_FOLDER}/*.*)
endif()
    ...
    if(NOT TARGET SDL2::SDL2_ttf)
        ...
        if(MINGW)
            ...
            if(SDL2PC_LIBRARIES)
                ...
                if(SDL2_TTF_MINGW_LIBRARY)
                    set(SDL2MAIN_LIBRARIES ${SDL2_TTF_MINGW_LIBRARY}
                                           ${SDL2MAIN_LIBRARIES})
                endif()
                if(SDL2_TTF_MWINDOWS_LIBRARY)
                    set(SDL2MAIN_LIBRARIES ${SDL2_TTF_MWINDOWS_LIBRARY}
                                           ${SDL2MAIN_LIBRARIES})
                endif()
            endif()
        endif()
