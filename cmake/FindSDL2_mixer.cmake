...
#
# SDL2::SDL2_mixer - Imported target to use for building a library
# SDL2_MIXER_FOUND - True if SDL2 was found. SDL2_MIXER_DYNAMIC - If we found a
# DLL version of SDL (meaning you might want to copy a DLL from
# SDL2::SDL2_mixer)
#
# 2020 Malte Kießling <mkalte@mkalte.me>: Added support for SDL2_MIXER
#
# Original Author: 2015 Ryan Pavlik <ryan.pavlik@gmail.com>
# <abiryan@ryand.mixer>
#
...
if(PKG_CONFIG_FOUND)
    pkg_search_module(SDL2PC QUIET sdl2_MIXER)
    if(SDL2PC_INCLUDE_DIRS)
        set(SDL2_MIXER_INCLUDE_HINTS ${SDL2PC_INCLUDE_DIRS})
    endif()
    if(SDL2PC_LIBRARY_DIRS)
        set(SDL2_MIXER_LIB_HINTS ${SDL2PC_LIBRARY_DIRS})
    endif()
endif()
...
find_library(
    SDL2_MIXER_LIBRARY
    NAMES SDL2_mixer SDL2_mixer
    HINTS ${SDL2_MIXER_LIB_HINTS}
    PATHS ${SDL2_MIXER_ROOT_DIR} ENV SDL2DIR ENV SDL2MIXERDIR
    PATH_SUFFIXES lib SDL2 ${SDL2_MIXER_LIB_PATH_SUFFIX})

set(_SDL2_MIXER_framework FALSE)
...
if(APPLE AND "${SDL2_MIXER_LIBRARY}" MATCHES "(/[^/]+)*.framework(/.*)?$")
    set(_SDL2_MIXER_framework TRUE)
    set(SDL2_MIXER_FRAMEWORK "${SDL2_MIXER_LIBRARY}")
    # Move up in the directory tree as required to get the framework directory.
    while("${SDL2_MIXER_FRAMEWORK}" MATCHES "(/[^/]+)*.framework(/.*)$"
          AND NOT "${SDL2_MIXER_FRAMEWORK}" MATCHES "(/[^/]+)*.framework$")
        get_filename_component(SDL2_MIXER_FRAMEWORK "${SDL2_MIXER_FRAMEWORK}"
                               DIRECTORY)
    endwhile()
    if("${SDL2_MIXER_FRAMEWORK}" MATCHES "(/[^/]+)*.framework$")
        ...
        find_path(
            ...
            NAMES SDL_MIXER.h # this file was introduced with SDL2
            HINTS "${SDL2_MIXER_FRAMEWORK}/Headers/")
    else()
...
find_path(
    SDL2_MIXER_INCLUDE_DIR
    NAMES SDL_mixer.h # sdl2 mixer
          SDL_mixer.h
    HINTS ${SDL2_MIXER_INCLUDE_HINTS}
    PATHS ${SDL2_MIXER_ROOT_DIR} ENV SDL2DIR ENV SDL2MIXERDIR
...
if(WIN32 AND SDL2_MIXER_LIBRARY)
    ...
    get_filename_component(SDL2_MIXER_RUNTIME_LIBRARY_FOLDER
                           ${SDL2_MIXER_RUNTIME_LIBRARY} DIRECTORY)
    file(GLOB SDL2_MIXER_RUNTIME_LIBRARY_ADDITIONALS
         ${SDL2_MIXER_RUNTIME_LIBRARY_FOLDER}/*.*)
endif()
...
find_package_handle_standard_args(
    SDL2_MIXER DEFAULT_MSG SDL2_MIXER_LIBRARY SDL2_MIXER_INCLUDE_DIR
    ...
    if(NOT TARGET SDL2::SDL2_mixer)
        ...
        if(WIN32 AND SDL2_MIXER_RUNTIME_LIBRARY)
            ...
            set_target_properties(
                SDL2::SDL2_mixer
                PROPERTIES IMPORTED_IMPLIB "${SDL2_MIXER_LIBRARY}"
            ...
            if(SDL2_MIXER_FRAMEWORK AND SDL2_MIXER_FRAMEWORK_NAME)
                ...
                set_target_properties(
                    ...
                    PROPERTIES
                        IMPORTED_LOCATION
                        "${SDL2_MIXER_FRAMEWORK}/${SDL2_MIXER_FRAMEWORK_NAME}")
            elseif(_SDL2_MIXER_framework AND SDL2_MIXER_LIBRARY MATCHES
                                             "(/[^/]+)*.framework$")
                ...
                set_target_properties(
                    ...
                    PROPERTIES IMPORTED_LOCATION
                               "${SDL2_MIXER_LIBRARY}/${CMAKE_MATCH_1}")
            else()
                # Handle non-frameworks (including non-Mac), as well as the case
                # that we're given the library inside of the framework
                set_target_properties(
                    SDL2::SDL2_mixer PROPERTIES IMPORTED_LOCATION
                                                "${SDL2_MIXER_LIBRARY}")
            endif()
            set_target_properties(
                SDL2::SDL2_mixer PROPERTIES INTERFACE_INCLUDE_DIRECTORIES
                                            "${SDL2_MIXER_INCLUDE_DIR}")
        endif()

        if(APPLE)
            # Need Cocoa here, is always a framework
            find_library(SDL2_MIXER_COCOA_LIBRARY Cocoa)
            list(APPEND SDL2_MIXER_EXTRA_REQUIRED SDL2_MIXER_COCOA_LIBRARY)
            if(SDL2_MIXER_COCOA_LIBRARY)
                set_target_properties(
                    SDL2::SDL2_mixer
        ...
        if(MINGW)
            ...
            if(SDL2PC_LIBRARIES)
                ...
                if(SDL2_MIXER_MINGW_LIBRARY)
                    set(SDL2MAIN_LIBRARIES ${SDL2_MIXER_MINGW_LIBRARY}
                                           ${SDL2MAIN_LIBRARIES})
                endif()
                if(SDL2_MIXER_MWINDOWS_LIBRARY)
                    set(SDL2MAIN_LIBRARIES ${SDL2_MIXER_MWINDOWS_LIBRARY}
                                           ${SDL2MAIN_LIBRARIES})
                endif()
            endif()
        endif()
...
mark_as_advanced(
    SDL2_MIXER_LIBRARY SDL2_MIXER_RUNTIME_LIBRARY SDL2_MIXER_INCLUDE_DIR
    SDL2_MIXER_COCOA_LIBRARY SDL2_MIXER_MINGW_LIBRARY
