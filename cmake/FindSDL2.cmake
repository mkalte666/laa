...
#
# SDL2::SDL2 - Imported target to use for building a library SDL2::SDL2main -
# Imported interface target to use if you want SDL and SDLmain. SDL2_FOUND -
# True if SDL2 was found. SDL2_DYNAMIC - If we found a DLL version of SDL
# (meaning you might want to copy a DLL from SDL2::SDL2)
#
...
find_library(
    SDL2_LIBRARY
    NAMES SDL2
    HINTS ${SDL2_LIB_HINTS}
    PATHS ${SDL2_ROOT_DIR} ENV SDL2DIR
...
if(APPLE AND "${SDL2_LIBRARY}" MATCHES "(/[^/]+)*.framework(/.*)?$")
    ...
    while("${SDL2_FRAMEWORK}" MATCHES "(/[^/]+)*.framework(/.*)$"
          AND NOT "${SDL2_FRAMEWORK}" MATCHES "(/[^/]+)*.framework$")
    ...
    if("${SDL2_FRAMEWORK}" MATCHES "(/[^/]+)*.framework$")
        ...
        find_path(
            SDL2_INCLUDE_DIR
            NAMES SDL_haptic.h # this file was introduced with SDL2
            HINTS "${SDL2_FRAMEWORK}/Headers/")
    else()
...
if(WIN32
   OR ANDROID
   OR IOS
   OR (APPLE AND NOT _sdl2_framework))
...
include(FindPackageHandleStandardArgs)
find_package_handle_standard_args(SDL2 DEFAULT_MSG SDL2_LIBRARY
                                  SDL2_INCLUDE_DIR ${SDL2_EXTRA_REQUIRED})
    ...
    if(NOT TARGET SDL2::SDL2)
        ...
        if(WIN32 AND SDL2_RUNTIME_LIBRARY)
            ...
            set_target_properties(
                SDL2::SDL2
                PROPERTIES IMPORTED_IMPLIB "${SDL2_LIBRARY}"
            ...
            if(SDL2_FRAMEWORK AND SDL2_FRAMEWORK_NAME)
                ...
                set_target_properties(
                    ...
                    PROPERTIES IMPORTED_LOCATION
                               "${SDL2_FRAMEWORK}/${SDL2_FRAMEWORK_NAME}")
            elseif(_sdl2_framework AND SDL2_LIBRARY MATCHES
                                       "(/[^/]+)*.framework$")
                ...
                set_target_properties(
                    SDL2::SDL2 PROPERTIES IMPORTED_LOCATION
                                          "${SDL2_LIBRARY}/${CMAKE_MATCH_1}")
            else()
                # Handle non-frameworks (including non-Mac), as well as the case
                # that we're given the library inside of the framework
                set_target_properties(SDL2::SDL2 PROPERTIES IMPORTED_LOCATION
                                                            "${SDL2_LIBRARY}")
            endif()
            set_target_properties(
                SDL2::SDL2 PROPERTIES INTERFACE_INCLUDE_DIRECTORIES
                                      "${SDL2_INCLUDE_DIR}")
        endif()
        ...
        if(APPLE)
            ...
            if(SDL2_COCOA_LIBRARY)
                set_target_properties(
                    SDL2::SDL2 PROPERTIES IMPORTED_LINK_INTERFACE_LIBRARIES
                                          ${SDL2_COCOA_LIBRARY})
            endif()
        endif()
        ...
        if(SDL2_SDLMAIN_LIBRARY)
            ...
            set_target_properties(
                SDL2::SDL2main_real PROPERTIES IMPORTED_LOCATION
                                               "${SDL2_SDLMAIN_LIBRARY}")
        ...
        if(MINGW)
            ...
            if(SDL2PC_LIBRARIES)
                ...
                if(SDL2_MINGW_LIBRARY)
                    set(SDL2MAIN_LIBRARIES ${SDL2_MINGW_LIBRARY}
                                           ${SDL2MAIN_LIBRARIES})
                endif()
                if(SDL2_MWINDOWS_LIBRARY)
                    set(SDL2MAIN_LIBRARIES ${SDL2_MWINDOWS_LIBRARY}
                                           ${SDL2MAIN_LIBRARIES})
                endif()
            endif()
            set_target_properties(
                SDL2::SDL2main PROPERTIES INTERFACE_COMPILE_DEFINITIONS
                                          "main=SDL_main")
        endif()
        set_target_properties(SDL2::SDL2main PROPERTIES INTERFACE_LINK_LIBRARIES
                                                        "${SDL2MAIN_LIBRARIES}")
    endif()
